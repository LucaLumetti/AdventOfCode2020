...
def state_to_str(p1,p2,g):
    str1 = ','.join([str(a) for a in p1])
    str2 = ','.join([str(b) for b in p2])
    return f"G{g};P1:{str1};P2:{str2}"

def check_history(p1,p2,g):
    s = state_to_str(p1,p2,g)
    if(s in history and history[s] == 1):
        return True
    history[s] = 1
    return False
...
def game(p1,p2, g):
    ...
    while(True):
        ...
        if(len(p1) >= a and len(p2) >= b):
            round_winner, _, __ = game(p1[0:a].copy(), p2[0:b].copy(), g+1)
        else:
