...
space = np.zeros((space_dim,space_dim,space_dim,space_dim))

for i in range(dim[0]):
    for j in range(dim[1]):
        space[half_space_dim,half_space_dim,half_space_dim-l+i,half_space_dim-u+j] = f[i][j]

def step(array, x,y,z,w):
    sx, sy, sz, sw = x-1,y-1,z-1,w-1
    ex, ey, ez, ew = x+1,y+1,z+1,w+1
    ...
    if(w == array.shape[2]-1): ew = w

    t = array[sx:ex+1, sy:ey+1, sz:ez+1, sw:ew+1].sum()
    if(array[x,y,z,w] == 1 and t in [3,4]): return True
    if(array[x,y,z,w] == 0 and t == 3): return True
    return False
        ...
        for y in range(lower_bound, upper_bound):
            for z in range(lower_bound, upper_bound):
                for w in range(lower_bound, upper_bound):
                    spc[k%2][x,y,z,w] = int(step(spc[(k+1)%2], x,y,z,w))
